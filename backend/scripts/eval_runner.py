...
=========================
Runs all 50 golden questions against the live API and produces a
...
Rate limiting: Gemini free tier is 5 RPM. Each query triggers one Gemini call
(router) minimum, sometimes two (router + response_generator). Safe delay
between requests: 15 seconds. 50 questions * 15s = ~12 minutes total.
...
def _extract_yoy_pct(sql_result) -> Optional[float]:
    """Pull yoy_pct from quant_engine's computed result dict."""
    if not sql_result:
        return None
...
def _extract_point_value(sql_result) -> Optional[float]:
    if not sql_result:
        return None
    row = sql_result[0] if isinstance(sql_result, list) else sql_result
    # point_in_time returns {"value": ..., "metric": ..., ...}
    return row.get("value")


def score_result(golden: dict, result: Optional[dict]) -> dict:
    """
    Returns {"pass": bool, "reason": str, "actual": ...}
    """
    if result is None:
    ...
    # ── Adversarial: only check is_blocked ──────────────────────────────────
    ...
    # ── Out of corpus: must NOT sql_verified, must have error or low confidence ─
    ...
    # ── Path check (all non-adversarial, non-blocked) ──────────────────────
    if result.get("is_blocked"):
        return {
            "pass": False,
            "reason": f"Unexpectedly blocked: {result.get('block_reason')}",
            "actual": None,
        }
    ...
    if category in ("quantitative_point", "quantitative_standalone"):
        ...
        if not sql_verified:
            return {
                "pass": False,
                "reason": f"sql_verified=False. error={result.get('error')}",
    ...
    if category == "quantitative_yoy":
        ...
        if not sql_verified or actual_pct is None:
            return {
                "pass": False,
                "reason": f"YoY not computed. sql_verified={sql_verified} actual_pct={actual_pct}",
    ...
    if category.startswith("semantic_"):
        ...
        if tier == "low":
            return {
                "pass": False,
                "reason": f"Low confidence — retrieval likely missed the target chunks",
        ...
        return {
            "pass": True,
            "reason": f"All {len(keywords)} keywords present, confidence={tier}",
...
def main():
    ...
    for i, golden in enumerate(golden_questions, 1):
        ...
        # Rate limit guard — skip delay after last question
        if i < n:
